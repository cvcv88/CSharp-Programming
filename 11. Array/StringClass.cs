...
namespace _11._Array
{
	...
	// 내부적으로 문자 배열을 통해서 구현됨(문자열 = 문자(char)의 배열)
	// 문자열은 텍스트를 나타내는 데 사용되는 char의 순차적 집합

	// 배열처럼 클래스로 만들어졌다. 배열이나 문자열은 크기를 확정 지을 수가 없어서(고정X),
	// struct로 만들지 못함.(적합하지 않음)
	...
	internal class StringClass
	{
		void Main1()
		{
			...
			// str1[1] = 'a'; // error : 문자열의 배열 접근은 읽기 전용

			// char 배열로 문자열을 표현
			char[] array = new char[5] { 'a', 'b', 'c', 'd', 'e' };
			Console.WriteLine(array); // output : abcde

			array.Contains('b'); // 있으면 true 없으면 false
			str.Contains('d'); // 문자열도 가능
		...
		// string은 클래스이기 때문에 얕은 복사가 기본이어야 할텐데 왜 깊은 복사가 기본이냐?
		// string은 기본으로 클래스이지만 값형식처럼 사용되어야 하기 때문에

		// string은 중간에 값을 바꾸지 못하기 때문에 값을 바꾸면 쓰레기가 계속 생긴다. -> 메모리 파편화
