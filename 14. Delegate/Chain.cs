...
namespace _14._Delegate
{
	internal class Chain
	{
		...
		// <델리게이트 체인>
		// 하나의 델리게이트 변수에 여러 개의 함수를 할당하는 것이 가능(추가할당)
		// +=, -= 연산자를 통해 할당을 추가하고 제거할 수 있음
		// = 연산자를 통해 할당할 경우 이전의 다른 함수들을 할당한 상황이 사라짐

		// 순서를 보장하지 않음, 순서가 중요해서 사용하는 기술 아님!
		...
		void Main()
		{
			...
			action -= Func1;    // 델리게이트 인스턴스에 참조되지 않은 함수를 제거하는 경우 해당 작업이 ^무시^됨

			action = Func1;     // 델리게이트 인스턴스에 = 을 통해 할당할 경우 이전의 참조된 상황이 사라짐(이벤트와 연관있는 부분)
			action();           // Func1 이 호출됨

			action -= Func2;    // 있으면 하나 빼고, 없으면 무시
			action += Func2;    // 있었다면 하나 다시 추가, 없었다면 하나 추가
			;
		}
